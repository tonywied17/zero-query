...
const routes = [ { path: '/', component: 'home-page' }, { path: '/counter', component: 'counter-page' }, { path: '/todos', component: 'todos-page' }, { path: '/contacts', component: 'contacts-page' }, { path: '/api', component: 'api-demo' }, { path: '/about', component: 'about-page' },
];
const store = $.store('main', { state: { todos: [], visits: 0, contacts: [ { id: 1, name: 'Alice Johnson', email: 'alice@example.com', role: 'Designer', status: 'online', favorite: true }, { id: 2, name: 'Bob Martinez', email: 'bob@example.com', role: 'Developer', status: 'offline', favorite: false }, { id: 3, name: 'Carol White', email: 'carol@example.com', role: 'Manager', status: 'online', favorite: true }, { id: 4, name: 'Dave Kim', email: 'dave@example.com', role: 'Designer', status: 'away', favorite: false }, { id: 5, name: 'Eve Torres', email: 'eve@example.com', role: 'Developer', status: 'online', favorite: false }, ], contactsAdded: 0, }, actions: { incrementVisits(state) { state.visits++; }, addTodo(state, text) { state.todos.push({ id: $.uuid(), text: text.trim(), done: false, createdAt: Date.now(), }); }, toggleTodo(state, id) { const todo = state.todos.find(t => t.id === id); if (todo) todo.done = !todo.done; }, removeTodo(state, id) { state.todos = state.todos.filter(t => t.id !== id); }, clearCompleted(state) { state.todos = state.todos.filter(t => !t.done); }, addContact(state, { name, email, role }) { state.contacts.push({ id: Date.now(), name, email, role, status: 'offline', favorite: false, }); state.contactsAdded++; }, deleteContact(state, id) { state.contacts = state.contacts.filter(c => c.id !== id); }, toggleFavorite(state, id) { const c = state.contacts.find(c => c.id === id); if (c) c.favorite = !c.favorite; }, cycleContactStatus(state, id) { const c = state.contacts.find(c => c.id === id); if (!c) return; const order = ['online', 'away', 'offline']; c.status = order[(order.indexOf(c.status) + 1) % 3]; }, }, getters: { todoCount: (state) => state.todos.length, doneCount: (state) => state.todos.filter(t => t.done).length, pendingCount: (state) => state.todos.filter(t => !t.done).length, contactCount: (state) => state.contacts.length, favoriteCount: (state) => state.contacts.filter(c => c.favorite).length, }, debug: true, });
$.component('home-page', { state: () => ({ greeting: '', signalDemo: 0, }), mounted() { const count = $.signal(0); const doubled = $.computed(() => count.value * 2); $.effect(() => { this.state.signalDemo = doubled.value; }); this._signalCount = count; const hour = new Date().getHours(); this.state.greeting = hour < 12 ? 'Good morning' : hour < 18 ? 'Good afternoon' : 'Good evening'; const store = $.getStore('main'); store.dispatch('incrementVisits'); }, incrementSignal() { if (this._signalCount) { this._signalCount.value++; } }, render() { const store = $.getStore('main'); return ` <div class="page-header"> <h1>${this.state.greeting} 👋</h1> <p class="subtitle">Welcome to your new <strong>zQuery</strong> app. Explore the pages to see different features in action.</p> </div> <div class="card-grid"> <div class="card card-accent"> <h3>⚡ Reactive Signals</h3> <p>Fine-grained reactivity with <code>signal()</code>, <code>computed()</code>, and <code>effect()</code>.</p> <div class="signal-demo"> <span class="signal-value">Doubled: ${this.state.signalDemo}</span> <button class="btn btn-sm" @click="incrementSignal">Increment Signal</button> </div> </div> <div class="card"> <h3>🔢 Counter</h3> <p>Component state, two-way binding with <code>z-model</code>, and event handling.</p> <a z-link="/counter" class="btn btn-outline">Try It →</a> </div> <div class="card"> <h3>✅ Todos</h3> <p>Global store with actions & getters. <strong>${store.getters.todoCount}</strong> items, <strong>${store.getters.doneCount}</strong> done.</p> <a z-link="/todos" class="btn btn-outline">Try It →</a> </div> <div class="card"> <h3>📇 Contacts</h3> <p>External templates &amp; styles via <code>templateUrl</code> / <code>styleUrl</code>. <strong>${store.getters.contactCount}</strong> contacts, <strong>${store.getters.favoriteCount}</strong> ★ favorited.</p> <a z-link="/contacts" class="btn btn-outline">Try It →</a> </div> <div class="card"> <h3>🌐 API Demo</h3> <p>Fetch data with <code>$.get()</code>, loading states, and <code>$.escapeHtml()</code>.</p> <a z-link="/api" class="btn btn-outline">Try It →</a> </div> </div> <div class="card card-muted"> <h3>📊 App Stats</h3> <div class="stats-grid"> <div class="stat-group"> <span class="stat-group-title">🏠 General</span> <div class="stat-group-values"> <div class="stat"> <span class="stat-value">${store.state.visits}</span> <span class="stat-label">Page Views</span> </div> </div> </div> <div class="stat-group"> <span class="stat-group-title">✅ Todos</span> <div class="stat-group-values"> <div class="stat"> <span class="stat-value">${store.getters.todoCount}</span> <span class="stat-label">Total</span> </div> <div class="stat"> <span class="stat-value">${store.getters.pendingCount}</span> <span class="stat-label">Pending</span> </div> <div class="stat"> <span class="stat-value">${store.getters.doneCount}</span> <span class="stat-label">Done</span> </div> </div> </div> <div class="stat-group"> <span class="stat-group-title">📇 Contacts</span> <div class="stat-group-values"> <div class="stat"> <span class="stat-value">${store.getters.contactCount}</span> <span class="stat-label">Total</span> </div> <div class="stat"> <span class="stat-value">${store.getters.favoriteCount}</span> <span class="stat-label">★ Favorited</span> </div> </div> </div> </div> <small class="muted">Stats powered by <code>$.store()</code> getters — visit count tracked globally.</small> </div> `; }
});
$.component('counter-page', { state: () => ({ count: 0, step: 1, history: [], }), increment() { this.state.count += this.state.step; this.state.history.push({ action: '+', value: this.state.step, result: this.state.count }); if (this.state.history.length > 8) this.state.history.shift(); }, decrement() { this.state.count -= this.state.step; this.state.history.push({ action: '−', value: this.state.step, result: this.state.count }); if (this.state.history.length > 8) this.state.history.shift(); }, reset() { this.state.count = 0; this.state.history = []; $.bus.emit('toast', { message: 'Counter reset!', type: 'info' }); }, render() { return ` <div class="page-header"> <h1>Counter</h1> <p class="subtitle">Component state, <code>@click</code> handlers, <code>z-model</code>, <code>z-class</code>, and <code>z-for</code>.</p> </div> <div class="card counter-card"> <div class="counter-display"> <span class="counter-value" z-class="{'negative': count < 0}">${this.state.count}</span> </div> <div class="counter-controls"> <button class="btn btn-danger" @click="decrement">− Subtract</button> <button class="btn btn-primary" @click="increment">+ Add</button> </div> <div class="counter-step"> <label>Step size: <input type="number" z-model="step" z-number min="1" max="100" class="input input-sm" /> </label> <button class="btn btn-ghost btn-sm" @click="reset">Reset</button> </div> </div> <div class="card card-muted" z-if="history.length > 0"> <h3>History</h3> <div class="history-list"> <span z-for="e in history" class="history-item">{{e.action}}{{e.value}} → <strong>{{e.result}}</strong></span> </div> </div> `; }
});
$.component('todos-page', { state: () => ({ newTodo: '', filter: 'all', search: '', filtered: [], total: 0, done: 0, pending: 0, }), mounted() { const store = $.getStore('main'); this._unsub = store.subscribe(() => this.setState({})); this._debouncedSearch = $.debounce((val) => { this.state.search = val; }, 300); }, destroyed() { if (this._unsub) this._unsub(); }, addTodo() { const text = this.state.newTodo.trim(); if (!text) return; $.getStore('main').dispatch('addTodo', text); this.state.newTodo = ''; this.state.search = ''; this.state.filter = 'all'; $.bus.emit('toast', { message: 'Todo added!', type: 'success' }); }, toggleTodo(id) { $.getStore('main').dispatch('toggleTodo', id); }, removeTodo(id) { $.getStore('main').dispatch('removeTodo', id); $.bus.emit('toast', { message: 'Todo removed', type: 'error' }); }, clearCompleted() { $.getStore('main').dispatch('clearCompleted'); $.bus.emit('toast', { message: 'Completed todos cleared', type: 'info' }); }, setFilter(f) { this.state.filter = f; }, onSearch(e) { this._debouncedSearch(e.target.value); }, render() { const store = $.getStore('main'); const todos = store.state.todos; const { filter, search } = this.state; let list = todos; if (filter === 'active') list = todos.filter(t => !t.done); if (filter === 'done') list = todos.filter(t => t.done); if (search) { const q = search.toLowerCase(); list = list.filter(t => t.text.toLowerCase().includes(q)); } this.state.filtered = list; this.state.total = store.getters.todoCount; this.state.done = store.getters.doneCount; this.state.pending = store.getters.pendingCount; return ` <div class="page-header"> <h1>Todos</h1> <p class="subtitle">Global store with <code>$.store()</code>, <code>z-for</code>, <code>z-class</code>, <code>z-if</code>, and <code>z-show</code>.</p> </div> <div class="card"> <form class="todo-form" @submit.prevent="addTodo"> <input type="text" z-model="newTodo" z-trim placeholder="What needs to be done?" class="input" z-ref="todoInput" /> <button type="submit" class="btn btn-primary">Add</button> </form> </div> <div class="card"> <div class="todo-toolbar"> <div class="todo-filters"> <button class="btn btn-sm" z-class="{'btn-primary': filter === 'all', 'btn-ghost': filter !== 'all'}" @click="setFilter('all')">All (${this.state.total})</button> <button class="btn btn-sm" z-class="{'btn-primary': filter === 'active', 'btn-ghost': filter !== 'active'}" @click="setFilter('active')">Active (${this.state.pending})</button> <button class="btn btn-sm" z-class="{'btn-primary': filter === 'done', 'btn-ghost': filter !== 'done'}" @click="setFilter('done')">Done (${this.state.done})</button> </div> <input type="text" placeholder="Search…" class="input input-sm" @input="onSearch" value="${$.escapeHtml(this.state.search)}" /> </div> <div z-if="filtered.length === 0" class="empty-state"> <p>${this.state.total === 0 ? 'No todos yet — add one above!' : 'No matching todos.'}</p> </div> <ul z-else class="todo-list"> <li z-for="t in filtered" class="todo-item {{t.done ? 'done' : ''}}"> <button class="todo-check" @click="toggleTodo('{{t.id}}')"></button> <span class="todo-text">{{$.escapeHtml(t.text)}}</span> <button class="todo-remove" @click="removeTodo('{{t.id}}')">✕</button> </li> </ul> <div class="todo-footer" z-show="done > 0"> <button class="btn btn-ghost btn-sm" @click="clearCompleted">Clear completed (${this.state.done})</button> </div> </div> `; }
});
$.component('api-demo', { state: () => ({ users: [], selectedUser: null, posts: [], loading: false, error: '', }), mounted() { this.fetchUsers(); }, async fetchUsers() { this.state.loading = true; this.state.error = ''; try { const res = await $.get('https://jsonplaceholder.typicode.com/users'); this.state.users = res.data.slice(0, 6); } catch (err) { this.state.error = 'Failed to load users. Check your connection.'; } this.state.loading = false; }, async selectUser(id) { this.state.selectedUser = this.state.users.find(u => u.id === Number(id)); this.state.loading = true; try { const res = await $.get(`https://jsonplaceholder.typicode.com/posts?userId=${id}`); this.state.posts = res.data.slice(0, 4); } catch (err) { this.state.error = 'Failed to load posts.'; } this.state.loading = false; $.bus.emit('toast', { message: `Loaded posts for ${this.state.selectedUser.name}`, type: 'success' }); }, clearSelection() { this.state.selectedUser = null; this.state.posts = []; }, render() { const { selectedUser } = this.state; return ` <div class="page-header"> <h1>API Demo</h1> <p class="subtitle">Fetching data with <code>$.get()</code>. Directives: <code>z-if</code>, <code>z-show</code>, <code>z-for</code>, <code>z-text</code>.</p> </div> <div class="card card-error" z-show="error"><p>⚠ <span z-text="error"></span></p></div> <div class="loading-bar" z-show="loading"></div> <div z-if="!selectedUser"> <div class="card"> <h3>Users</h3> <p class="muted">Click a user to fetch their posts.</p> <div class="user-grid" z-if="users.length > 0"> <button z-for="u in users" class="user-card" @click="selectUser({{u.id}})"> <strong>{{u.name}}</strong> <small>@{{u.username}}</small> <small class="muted">{{u.company.name}}</small> </button> </div> <p z-else z-show="!loading">No users loaded.</p> </div> </div> <div z-else> <div class="card"> <div class="user-detail-header"> <div> <h3>${selectedUser ? $.escapeHtml(selectedUser.name) : ''}</h3> <p class="muted">${selectedUser ? `@${$.escapeHtml(selectedUser.username)} · ${$.escapeHtml(selectedUser.email)}` : ''}</p> </div> <button class="btn btn-ghost btn-sm" @click="clearSelection">← Back</button> </div> </div> <div class="card"> <h3>Recent Posts</h3> <div class="posts-list" z-if="posts.length > 0"> <article z-for="p in posts" class="post-item"> <h4>{{p.title}}</h4> <p>{{p.body.substring(0, 120)}}…</p> </article> </div> <p z-else class="muted" z-show="!loading">No posts found.</p> </div> </div> `; }
});
$.component('about-page', { state: () => ({ theme: 'dark', }), mounted() { this.state.theme = $.storage.get('theme') || 'dark'; }, toggleTheme() { const next = this.state.theme === 'dark' ? 'light' : 'dark'; this.state.theme = next; document.documentElement.setAttribute('data-theme', next); $.storage.set('theme', next); $.bus.emit('toast', { message: `Switched to ${next} theme`, type: 'info' }); }, render() { return ` <div class="page-header"> <h1>About</h1> <p class="subtitle">Built with zQuery v${$.version} — a zero-dependency frontend library.</p> </div> <div class="card"> <h3>🎨 Theme</h3> <p>Toggle between dark and light mode. Persisted to <code>localStorage</code> via <code>$.storage</code>.</p> <div class="theme-toggle"> <span>Current: <strong>${this.state.theme}</strong></span> <button class="btn btn-outline" @click="toggleTheme">${this.state.theme === 'dark' ? '☀️ Light Mode' : '🌙 Dark Mode'}</button> </div> </div> <div class="card"> <h3>🧰 Features Used in This App</h3> <div class="feature-grid"> <div class="feature-item"> <strong>$.component()</strong> <span>Reactive components with state, lifecycle hooks, and template rendering</span> </div> <div class="feature-item"> <strong>$.router()</strong> <span>SPA routing with history mode, z-link navigation, and fallback pages</span> </div> <div class="feature-item"> <strong>$.store()</strong> <span>Centralized state management with actions, getters, and subscriptions</span> </div> <div class="feature-item"> <strong>$.get()</strong> <span>HTTP client for fetching JSON APIs with async/await</span> </div> <div class="feature-item"> <strong>$.signal() / $.computed()</strong> <span>Fine-grained reactive primitives for derived state</span> </div> <div class="feature-item"> <strong>$.bus</strong> <span>Event bus for cross-component communication (toast notifications)</span> </div> <div class="feature-item"> <strong>$.storage</strong> <span>localStorage wrapper for persisting user preferences</span> </div> <div class="feature-item"> <strong>$.debounce()</strong> <span>Debounced search input in the todos filter</span> </div> <div class="feature-item"> <strong>$.escapeHtml()</strong> <span>Safe rendering of user-generated and API content</span> </div> <div class="feature-item"> <strong>$.uuid()</strong> <span>Unique ID generation for new todo items</span> </div> <div class="feature-item"> <strong>z-model / z-ref</strong> <span>Two-way data binding and DOM element references</span> </div> <div class="feature-item"> <strong>templateUrl / styleUrl</strong> <span>External HTML templates and CSS with auto-scoping (contacts page)</span> </div> <div class="feature-item"> <strong>z-if / z-for / z-show</strong> <span>Structural directives for conditional &amp; list rendering</span> </div> <div class="feature-item"> <strong>z-bind / z-class / z-style</strong> <span>Dynamic attributes, classes, and inline styles</span> </div> <div class="feature-item"> <strong>$.on()</strong> <span>Global delegated event listeners for the hamburger menu</span> </div> </div> </div> <div class="card card-muted"> <h3>📚 Next Steps</h3> <ul class="next-steps"> <li>Read the <a href="https://z-query.com/docs" target="_blank" rel="noopener">full documentation</a></li> <li>Explore the <a href="https://github.com/tonywied17/zero-query" target="_blank" rel="noopener">source on GitHub</a></li> <li>Run <code>npx zquery bundle</code> to build for production</li> <li>Run <code>npx zquery dev</code> for live-reload development</li> </ul> </div> `; }
});
$.component('contacts-page', { templateUrl: 'cli/scaffold/scripts/components/contacts/contacts.html', styleUrl: 'cli/scaffold/scripts/components/contacts/contacts.css', state: () => ({ contacts: [], showForm: false, newName: '', newEmail: '', newRole: 'Developer', nameError: '', emailError: '', selectedId: null, confirmDeleteId: null, totalAdded: 0, favoriteCount: 0, }), mounted() { const store = $.getStore('main'); this._syncFromStore(store); this._unsub = store.subscribe(() => this._syncFromStore(store)); }, destroyed() { if (this._unsub) this._unsub(); }, _syncFromStore(store) { this.state.contacts = store.state.contacts; this.state.totalAdded = store.state.contactsAdded; this.state.favoriteCount = store.getters.favoriteCount; }, toggleForm() { this.state.showForm = !this.state.showForm; if (!this.state.showForm) this._clearForm(); }, _validateName(name) { if (!name) return 'Name is required.'; if (name.length < 2) return 'Name must be at least 2 characters.'; return ''; }, _validateEmail(email) { if (!email) return 'Email is required.'; if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return 'Enter a valid email address.'; const store = $.getStore('main'); if (store.state.contacts.some(c => c.email.toLowerCase() === email.toLowerCase())) { return 'A contact with this email already exists.'; } return ''; }, validateName() { this.state.nameError = this._validateName(this.state.newName.trim()); }, validateEmail() { this.state.emailError = this._validateEmail(this.state.newEmail.trim()); }, addContact() { const name = this.state.newName.trim(); const email = this.state.newEmail.trim(); const nameError = this._validateName(name); const emailError = this._validateEmail(email); this.state.nameError = nameError; this.state.emailError = emailError; if (nameError || emailError) return; $.getStore('main').dispatch('addContact', { name, email, role: this.state.newRole, }); this._clearForm(); this.state.showForm = false; $.bus.emit('toast', { message: `${name} added!`, type: 'success' }); }, toggleFavorite(id) { $.getStore('main').dispatch('toggleFavorite', Number(id)); }, selectContact(id) { this.state.selectedId = this.state.selectedId === Number(id) ? null : Number(id); this.state.confirmDeleteId = null; }, confirmDelete(id) { this.state.confirmDeleteId = Number(id); }, cancelDelete() { this.state.confirmDeleteId = null; }, deleteContact(id) { const numId = Number(id); const store = $.getStore('main'); const c = store.state.contacts.find(c => c.id === numId); store.dispatch('deleteContact', numId); this.state.selectedId = null; this.state.confirmDeleteId = null; $.bus.emit('toast', { message: `${c ? c.name : 'Contact'} removed`, type: 'error' }); }, cycleStatus(id) { $.getStore('main').dispatch('cycleContactStatus', Number(id)); }, _clearForm() { this.state.newName = ''; this.state.newEmail = ''; this.state.newRole = 'Developer'; this.state.nameError = ''; this.state.emailError = ''; },
});
$.component('not-found', { render() { const router = $.getRouter(); return ` <div class="page-header center"> <h1>404</h1> <p class="subtitle">The page <code>${$.escapeHtml(router.current?.path || '')}</code> was not found.</p> <a z-link="/" class="btn btn-primary">← Go Home</a> </div> `; }
});
const router = $.router({ el: '#app', routes, fallback: 'not-found', mode: 'history'
});
router.onChange((to) => { $.all('.nav-link').removeClass('active'); $.all(`.nav-link[z-link="${to.path}"]`).addClass('active'); closeMobileMenu();
});
const sidebar = $.id('sidebar');
const overlay = $.id('overlay');
const toggle = $.id('menu-toggle');
function openMobileMenu() { sidebar.classList.add('open'); overlay.classList.add('visible'); toggle.classList.add('active');
}
function closeMobileMenu() { sidebar.classList.remove('open'); overlay.classList.remove('visible'); toggle.classList.remove('active');
}
$.on('click', '#menu-toggle', () => { sidebar.classList.contains('open') ? closeMobileMenu() : openMobileMenu();
});
$.on('click', '#overlay', closeMobileMenu);
$.on('keydown', (e) => { if (e.key === 'Escape') closeMobileMenu();
});
$.bus.on('toast', ({ message, type = 'info' }) => { const container = $.id('toasts'); const toast = $.create('div'); toast.className = `toast toast-${type}`; toast.textContent = message; container.appendChild(toast); setTimeout(() => { toast.classList.add('toast-exit'); setTimeout(() => toast.remove(), 300); }, 3000);
});
$.ready(() => { const versionEl = $.id('nav-version'); if (versionEl) versionEl.textContent = 'v' + $.version; const savedTheme = $.storage.get('theme'); if (savedTheme) document.documentElement.setAttribute('data-theme', savedTheme); const current = window.location.pathname; $.all(`.nav-link[z-link="${current}"]`).addClass('active'); console.log('⚡ {{NAME}} — powered by zQuery v' + $.version);
});
})();
